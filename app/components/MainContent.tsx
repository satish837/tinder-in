...
export default function MainContent({ initialSelected }: { initialSelected?: string | null }) {
  ...
  const toggle = (id?: string) => {
    ...
    if (selected !== id) {
      // open
      router.push(`/` + (id === 'emotional' ? 'emotional-gps' : ''));
      setTimeout(() => {
        const el = document.getElementById(`details-${id}`);
        if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 320);
    } else {
      // closing - go back to base path
      router.push(window.location.pathname === `/${id === 'emotional' ? 'emotional-gps' : ''}` ? '/' : window.location.pathname);
    }
  };
  ...
  // When initialSelected changes (e.g., route /emotional-gps), ensure selection is set
  useEffect(() => {
    if (initialSelected) {
      setSelected(initialSelected);
      setTimeout(() => {
        const el = document.getElementById(`details-${initialSelected}`);
        if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 300);
    }
  }, [initialSelected]);
      ...
      <div className="mt-0 grid gap-6">
        <section className="bg-pinkLight/20 rounded-xl p-4 md:p-6">
          ...
          <p className="mt-3 text-gray-700">
            Dr. Chandni’s Tip: Take your time, a steady pace helps your mind and heart align. Don’t rush feelings; set the tempo
        ...
        <section className="rounded-xl p-4 md:p-6 border border-pinkLight/40">
          ...
          <p className="mt-2 text-gray-700">
            Make space for yourself by letting go of old memories. Letting go doesn’t mean forgetting; it’s about making space for your
            next chapter. Dropping keepsakes, even symbolically, helps your mind release old attachments.
          </p>
      ...
      <section className="w-full mx-auto px-4 mt-4 md:mt-8">
        ...
        <div className="max-w-6xl mx-auto my-12 md:my-20 grid gap-12 md:gap-20">
          {(
            ...
            <div key={c.id}>
              <InfoCard id={c.id} title={c.title} subtitle={c.subtitle} image={c.image} reverse={c.reverse} onActivate={(id) => toggle(id)} selected={selected === c.id} />

              {/* Render details panel immediately after the clicked card */}
              <div id={`details-${c.id}`} className={`${selected === c.id ? 'mt-0' : ''}`}>
                <div className="w-full max-w-6xl mx-auto px-4 md:px-0">
                  <div className={`overflow-hidden transition-[max-height] duration-500 ease-in-out`} style={{ maxHeight: selected === c.id ? undefined as any : 0 }} aria-hidden={selected !== c.id}>
                    <div className="pt-0 pb-8">
                      {selected === c.id && (
                        <div className="bg-white/70 rounded-xl-2 md:rounded-xl shadow-card border border-[#bd084d] p-4 md:p-6 lg:p-8">
                          <div className="flex justify-end">
                            <button onClick={() => setSelected(null)} className="text-sm text-gray-500 underline">Close</button>
                          </div>
                          {c.content}
                        </div>
                      )}
